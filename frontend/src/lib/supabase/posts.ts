...
function transformPostData(post: PostWithDetails): PostData {
  return {
    ...
    author: {
      ...
      tagline: post.author.github_url.split('/').pop() || '', // Use GitHub username as tagline
...
export async function getPosts(limit: number = 20, offset: number = 0): Promise<PostData[]> {
